...
fn getUnicodeVersions(allocator: Allocator) ![]const Version {
    var result = ArrayList(Version).empty;

    var file = try std.fs.cwd().openFile("DerivedAge.txt", .{});
    defer file.close();

    const buffer = try allocator.alloc(u8, max_line_len);
    ...
    while (reader.takeDelimiterExclusive('\n')) |line| {
        ...
        if (std.mem.startsWith(u8, line, needle)) {
            ...
            try result.append(allocator, try Version.parse(version));
        }
    } else |e| switch (e) {
        error.EndOfStream => {},
        else => return e,
...
fn writeUnicodeVersions(versions: []const Version) !void {
    ...
    const indent = "    ";
    for (versions) |v| {
        try writer.print(indent ++ "comptime Version.parse(\"{f}\"),\n", .{v});
    }
